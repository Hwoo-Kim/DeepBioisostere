...
def main():
    ...
    # Transformation frequency filtering
    if args.min_trans_count > 0:
        print(f"Filtering transformations with count < {args.min_trans_count}...")
        
        # Count unique (REF-CID, PRB-CID, ASSAY-ID) tuples for each transformation
        trans_counts = df.drop_duplicates(
            subset=["OLD-FRAG", "NEW-FRAG", "REF-CID", "PRB-CID", "ASSAY-ID"]
        ).groupby(["OLD-FRAG", "NEW-FRAG"]).size()

        # Get transformations that meet the minimum count
        frequent_trans = trans_counts[trans_counts >= args.min_trans_count]
        
        print(f"Found {len(frequent_trans)} transformations that appear >= {args.min_trans_count} times.")

        # Filter the original dataframe to keep only rows with frequent transformations
        df_multi_indexed = df.set_index(["OLD-FRAG", "NEW-FRAG"])
        frequent_trans_index = frequent_trans.index
        df = df_multi_indexed.loc[df_multi_indexed.index.isin(frequent_trans_index)].reset_index()
        
        print(f"Pairs remaining after frequency filtering: {len(df)}")
    ...
    output_path = args.data_dir / "data_revised.csv"
    print(f"Saving final data to {output_path}")
    df.to_csv(output_path, sep="\t", index_label="INDEX")
    print("Done.")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Generate keys for training with transformation frequency filtering.")
    parser.add_argument(
        "pair_files",
        nargs="+",
        metavar="PAIR_FILE",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum frequency for a transformation to be included. Set to 0 to disable."
    )
