...
class Generator:
    ...
    def __init__(
        ...
        num_sample_each_mol: Union[int, str] = 100,
        device: Union[torch.device, str] = "cpu",
        num_cores: int = 4,
        batch_size: int = 512,
        new_frag_type: str = "all",
        ...
        **kwargs,
        ...
        # Resolve the fragment library lazily, so callers need not know where
        # it lives: a source checkout, $DEEPBIOISOSTERE_ASSET_DIR and the
        # Hugging Face Hub all work. See deepbioisostere.assets.
        if processed_frag_dir is None:
            from .assets import resolve_fragment_library

            processed_frag_dir = resolve_fragment_library()
