...
class DeepBioisostere(nn.Module):
    ...
    def from_pretrained(
        ...
        properties: List[PROPERTY],
        ablation: bool = False,
        local_dir: Union[str, pathlib.Path, None] = None,
        **kwargs,
    ):
        """Load a published checkpoint for ``properties``, fetching it if needed.

        Args:
            properties: property names to condition on, any order. One of the
                published combinations; see
                ``deepbioisostere.assets.AVAILABLE_PROPERTY_SETS``.
            ablation: load the ``use_subgraph_AMPN=False`` variant. This changes
                the architecture (it adds the ``upscaler`` layer), so it is
                threaded into construction rather than only selecting a
                filename -- getting it wrong surfaces as a state_dict key
                mismatch.
            local_dir: directory holding the ``.pt`` files. When omitted, the
                checkpoint is resolved via ``$DEEPBIOISOSTERE_ASSET_DIR`` or
                downloaded from the Hugging Face Hub and cached.
            **kwargs: forwarded to ``from_trained_model`` as model args.
        """
        from .assets import resolve_checkpoint

        path = resolve_checkpoint(
            properties, ablation=ablation, local_dir=local_dir
        )
        kwargs.setdefault("use_subgraph_AMPN", not ablation)
        return cls.from_trained_model(path, properties=list(properties), **kwargs)
    ...
    def restore(self, path_to_model: Union[str, pathlib.Path]):
        ...
        # The published checkpoints are plain OrderedDict state dicts, so
        # weights_only=True is both safe and correct here. It is passed
        # explicitly because torch flipped this default in 2.6.
        if self.device == torch.device("cpu"):
            model_params = torch.load(
                path_to_model, map_location=torch.device("cpu"), weights_only=True
            )
        else:
            model_params = torch.load(path_to_model, weights_only=True)
